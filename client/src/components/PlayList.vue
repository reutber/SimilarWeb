<template>
  <div class="container-playlist">    
            <div class="playlist-form">
                <div class="form">
                    <input class="url-input" v-model="newItem.url" v-on:keyup.enter='handleAddNewItem' placeholder="Enter Video Id"/>
                    <button @click='handleAddNewItem' :disabled="!enableSubmitButton">Add</button>
                </div>    
                   <div class="playlist">         
                        <div v-for="video in videoPlayList" :key="video.date">
                                <play-list-item :video="video"/>
                        </div>
                 </div>
            </div>                          
            <youtube :video-id="firstVideo.id" :player-vars="playerVars" ref="youtube" v-show="renderComponent"></youtube>             
    </div>  
</template>
<script>
   ...
   methods:{       
        ...
        handleAddNewItem(){
            if(this.newItem && this.newItem.url.length > 0){              
                ...
                this.$store.dispatch({type: PLAYLIST_STORE.ACTIONS.ADD_NEW_ITEM, videoItem: this.newItem });
            }
        },
        ended(event){
        ...
        removeEnventOnVideoEnded(){
            this.$refs.youtube.player.removeEventListener("onStateChange", this.ended);
        },
        getEmptyItem(){
            return {
                url: '', 
    ...
    computed: {
        ...
        firstVideo(){
            return this.videoPlayList[0] ? this.videoPlayList[0] : this.getEmptyItem();           
        },       
        enableSubmitButton(){
